...
describe('Operatable', () => {
  let test;
  before(async () => {
    test = await deployContract(ownerWallet, 'OperatorableTest', [
      ownerWallet.address,
    ]);
  });
  ...
  it('Should not be able to run the onlyOperatorable function as not the operator', async () => {
    await expect(
      test.connect(otherWallet).testOperatorable(),
    ).to.be.revertedWithCustomError(test, 'NotOperator');
  });
