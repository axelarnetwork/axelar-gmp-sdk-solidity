...
describe('InterchainAddressTracker', () => {
    ...
    before(async () => {
        ...
        interchainAddressTracker = await deployContract(ownerWallet, 'TestInterchainAddressTracker', [
            chainName,
            defaultChains,
            defaultAddresses,
        ]);
    ...
    it('check internal constants', async () => {
        const interchainAddressTracker = await deployContract(ownerWallet, 'TestInterchainAddressTracker', [
            chainName,
            [],
            [],
        ]);
    ...
    it('Should be able to check trusted addresses properly', async () => {
        expect(await interchainAddressTracker.isTrustedAddress(otherChain, otherRemoteAddress)).to.equal(false);
    });

    it('Should not be able to add a trusted address as not the owner', async () => {
        await expect(
            interchainAddressTracker.connect(otherWallet).setTrustedAddress(otherChain, otherRemoteAddress),
    ...
    it('Should be able to add a trusted address as the owner', async () => {
        await expect(interchainAddressTracker.setTrustedAddress(otherChain, otherRemoteAddress))
            .to.emit(interchainAddressTracker, 'TrustedAddressSet')
            .withArgs(otherChain, otherRemoteAddress);
        expect(await interchainAddressTracker.trustedAddress(otherChain)).to.equal(otherRemoteAddress);
    });

    it('Should revert on setting a trusted address with an empty chain name', async () => {
        await expect(interchainAddressTracker.setTrustedAddress('', otherRemoteAddress)).to.be.revertedWithCustomError(
    ...
    it('Should revert on setting a trusted address with an invalid remote address', async () => {
        await expect(interchainAddressTracker.setTrustedAddress(otherChain, '')).to.be.revertedWithCustomError(
            interchainAddressTracker,
            'ZeroStringLength',
        );
    });
    ...
    it('Should be able to check trusted addresses properly.', async () => {
        expect(await interchainAddressTracker.isTrustedAddress(otherChain, otherRemoteAddress)).to.equal(true);
    });
    ...
    it('Should revert on removing a custom remote address with an empty chain name', async () => {
        await expect(interchainAddressTracker.removeTrustedAddress('')).to.be.revertedWithCustomError(
            interchainAddressTracker,
            'ZeroStringLength',
        );
    });
