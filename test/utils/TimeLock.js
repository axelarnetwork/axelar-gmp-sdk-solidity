...
describe('TimeLock', async () => {
    ...
    it('should revert when scheduling a timelock with invalid hash', async () => {
        ...
        await expect(timeLock.scheduleSetNumByHash(HashZero, eta)).to.be.revertedWithCustomError(
            timeLock,
            'InvalidTimeLockHash',
        );
    });
    ...
    it('should revert when scheduling the same timelock twice', async () => {
        ...
        await expect(timeLock.scheduleSetNum(num, eta)).to.be.revertedWithCustomError(
            timeLock,
    ...
    it('should revert when cancelling a timelock with invalid hash', async () => {
        await expect(timeLock.cancelSetNumByHash(HashZero)).to.be.revertedWithCustomError(
            timeLock,
            'InvalidTimeLockHash',
        );
    });
    ...
    it('should revert on executing a timelock with invalid hash', async () => {
        ...
        await expect(timeLock.setNumByHash(HashZero, num)).to.be.revertedWithCustomError(
            timeLock,
            'InvalidTimeLockHash',
        );
    });
    ...
    it('should revert on executing a timelock that has never been created', async () => {
        ...
        await expect(timeLock.setNum(num)).to.be.revertedWithCustomError(timeLock, 'InvalidTimeLockHash');
    });
    ...
    it('should revert on executing a timelock if buffer period has not passed', async () => {
        ...
        await expect(timeLock.setNum(num)).to.be.revertedWithCustomError(timeLock, 'TimeLockNotReady');
    });

    it('should execute timelock if buffer period has passed', async () => {
        const fastBuffer = 5;
        timeLock = await timeLockFactory.deploy(fastBuffer).then((d) => d.deployed());
        ...
        await expect(timeLock.setNum(num)).to.emit(timeLock, 'NumUpdated').withArgs(num);
