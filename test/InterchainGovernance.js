...
describe('InterchainGovernance', () => {
  ...
  before(async () => {
    [ownerWallet, governanceAddress, gatewayAddress] =
      await ethers.getSigners();

    interchainGovernanceFactory = await ethers.getContractFactory(
      'TestInterchainGovernance',
      ownerWallet,
    );
  ...
  beforeEach(async () => {
    ...
    targetInterface = new ethers.utils.Interface(
      targetContract.interface.fragments,
    );
  ...
  it('should revert on invalid command', async () => {
    ...
    await expect(
      interchainGovernance.executeProposalAction(
        governanceChain,
        governanceAddress.address,
        payload,
      ),
  ...
  it('should withdraw native ether from governance', async () => {
    ...
    await expect(tx)
      ...
      .to.changeEtherBalance(target, nativeValue);
  });
  ...
  it('should execute an existing proposal', async () => {
    ...
    const executionTimestamp = (await ethers.provider.getBlock(tx.blockNumber))
      .timestamp;
