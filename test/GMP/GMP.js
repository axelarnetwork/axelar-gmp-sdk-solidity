...
describe('GMP', () => {
    ...
    before(async () => {
        ...
        sourceChainSenderFactory = await ethers.getContractFactory('SourceChainSender', ownerWallet);
        destinationChainReceiverFactory = await ethers.getContractFactory('DestinationChainReceiver', ownerWallet);
    });
        ...
        describe('Call Contract with Token', () => {
            beforeEach(async () => {
                ...
                sourceChainSwapCaller = await sourceChainSwapCallerFactory
                    .deploy(sourceChainGateway.address, destinationChain, destinationChainSwapExecutable.address)
                    .then((d) => d.deployed());
                ...
                await (await tokenA.connect(ownerWallet).mint(userWallet.address, 1e9)).wait();
            });
        ...
        describe('Call Contract', () => {
            beforeEach(async () => {
                ...
                sourceChainSender = await sourceChainSenderFactory
                    .deploy(sourceChainGateway.address, destinationChain, destinationChainReceiver.address)
                    .then((d) => d.deployed());
            });
            ...
            it('should revert without gateway approval', async () => {
                ...
                await expect(receive).to.be.revertedWithCustomError(destinationChainReceiver, 'NotApprovedByGateway');
            });
            ...
            it('should call contract on another chain', async () => {
                ...
                await expect(receive).to.emit(destinationChainReceiver, 'Received').withArgs(num);
            });
        });
    });
});
