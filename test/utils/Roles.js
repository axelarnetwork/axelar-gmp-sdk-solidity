...
describe('Roles', () => {
    ...
    before(async () => {
        ...
        testRolesFactory = await ethers.getContractFactory('TestRoles', ownerWallet);
    });
    ...
    describe('negative tests', () => {
        beforeEach(async () => {
            ...
            testRoles = await testRolesFactory.deploy(accounts, roleSets).then((d) => d.deployed());
        });
        ...
        it('should revert when non-role account calls onlyRole function', async () => {
            ...
            await expectRevert(
                (gasOptions) => testRoles.connect(userWallet).setNum(num, role, gasOptions),
                testRoles,
                'MissingRole',
                {
        ...
        it('should not revert when role account calls onlyRole function', async () => {
            ...
            await expect(testRoles.connect(ownerWallet).setNum(num, role)).to.emit(testRoles, 'NumSet').withArgs(num);
        });
        ...
        it('should revert when non-role account calls withAllTheRoles function', async () => {
            ...
            await expectRevert(
                (gasOptions) => testRoles.connect(userWallet).setNumWithAllRoles(num, roles, gasOptions),
                testRoles,
                'MissingAllRoles',
                {
                    account: userWallet.address,
        ...
        it('should not revert when role account calls withAllTheRoles function', async () => {
            ...
            await expect(testRoles.connect(ownerWallet).setNumWithAllRoles(num, roles))
        ...
        it('should revert when non-role account calls withAnyOfRoles function', async () => {
            ...
            await expectRevert(
                (gasOptions) => testRoles.connect(userWallet).setNumWithAnyRoles(num, roles, gasOptions),
                testRoles,
                'MissingAnyOfRoles',
                {
                    account: userWallet.address,
        ...
        it('should not revert when role account calls withAnyOfRoles function', async () => {
            ...
            await expect(testRoles.connect(ownerWallet).setNumWithAnyRoles(num, roles))
                .to.emit(testRoles, 'NumSet')
                .withArgs(num);
        });
    });
    ...
    describe('negative tests for role transfers', () => {
        ...
        it('should revert on acceptRoles if called by an account without all the proposed roles', async () => {
            ...
            await testRoles.proposeRoles(userWallet.address, roles).then((tx) => tx.wait());

            await expectRevert(
                (gasOptions) => testRoles.connect(ownerWallet).acceptRoles(ownerWallet.address, roles, gasOptions),
                testRoles,
        ...
        it('should revert on transferRoles if called by an account without all the proposed roles', async () => {
            ...
            await testRoles.transferRoles(userWallet.address, [3]).then((tx) => tx.wait());

            await expectRevert(
                (gasOptions) => testRoles.connect(ownerWallet).transferRoles(userWallet.address, roles, gasOptions),
                testRoles,
                'MissingAllRoles',
                {
        ...
        it('should revert on transferRoles if transferred to an invalid account', async () => {
            ...
            await expectRevert(
                (gasOptions) => testRoles.transferRoles(ownerWallet.address, roles, gasOptions),
                testRoles,
                'InvalidProposedAccount',
                {
                    account: ownerWallet.address,
        ...
        it('should revert on proposeRoles if called by an account without all the proposed roles', async () => {
            ...
            await expectRevert(
                (gasOptions) => testRoles.connect(ownerWallet).proposeRoles(userWallet.address, roles, gasOptions),
                testRoles,
                'MissingAllRoles',
                {
                    account: ownerWallet.address,
        ...
        it('should revert on proposeRoles if proposed to an invalid account', async () => {
            ...
            await expectRevert(
                (gasOptions) => testRoles.connect(ownerWallet).proposeRoles(AddressZero, roles, gasOptions),
                testRoles,
                'InvalidProposedAccount',
                {
                    account: AddressZero,
            ...
            await expectRevert(
                (gasOptions) => testRoles.connect(ownerWallet).proposeRoles(ownerWallet.address, roles, gasOptions),
                testRoles,
                'InvalidProposedAccount',
                {
                    account: ownerWallet.address,
        ...
        it('should revert on acceptRoles if called with incorrect proposed roles', async () => {
            ...
            await expectRevert(
                (gasOptions) =>
                    testRoles.connect(userWallet).acceptRoles(ownerWallet.address, incorrectRoles, gasOptions),
                testRoles,
                'InvalidProposedRoles',
                {
                    fromAccount: ownerWallet.address,
    ...
    describe('positive tests', () => {
        ...
        it('should return if an account has a specific role', async () => {
            const hasRole = await testRoles.hasRole(ownerWallet.address, roleSets[0][0]);

            expect(hasRole).to.be.true;

            const hasRoleNegative = await testRoles.hasRole(userWallet.address, roleSets[0][0]);

            expect(hasRoleNegative).to.be.false;
        });

        it('should return if an account has all the roles', async () => {
            const hasAllTheRoles = await testRoles.hasAllTheRoles(ownerWallet.address, roleSets[0]);

            expect(hasAllTheRoles).to.be.true;

            const hasAllTheRolesNegative = await testRoles.hasAllTheRoles(userWallet.address, roleSets[0]);

            expect(hasAllTheRolesNegative).to.be.false;
        });

        it('should return if an account has any of the roles', async () => {
            const hasAnyRoles = await testRoles.hasAnyOfRoles(ownerWallet.address, roleSets[0]);

            expect(hasAnyRoles).to.be.true;

            const hasAnyRolesNegative = await testRoles.hasAnyOfRoles(userWallet.address, roleSets[0]);
        ...
        it('should add and remove roles', async () => {
            ...
            expect(await testRoles.getAccountRoles(userWallet.address)).to.equal(accountRoles);
        ...
        it('should transfer roles in one step', async () => {
            ...
            expect(await testRoles.getAccountRoles(userWallet.address)).to.equal(accountRoles); // 6 is the binary representation of roles [1, 2]
        ...
        it('should propose new roles and accept roles', async () => {
            ...
            expect(await testRoles.getProposedRoles(ownerWallet.address, userWallet.address)).to.equal(accountRoles); // 12 is the binary representation of roles [2, 3]
            ...
            expect(await testRoles.getProposedRoles(ownerWallet.address, userWallet.address)).to.equal(0); // cleared proposed roles
        ...
        it('should add and remove single role', async () => {
            ...
            expect(await testRoles.getAccountRoles(userWallet.address)).to.equal(1 << role); // 4 is the binary representation of roles [2]
        ...
        it('should transfer a single role in one step', async () => {
            ...
            await expect(testRoles.transferRole(ownerWallet.address, userWallet.address, role))
                .to.emit(testRoles, 'RolesRemoved')
                .withArgs(ownerWallet.address, accountRoles)
                .to.emit(testRoles, 'RolesAdded')
                .withArgs(userWallet.address, accountRoles);
            ...
            expect(await testRoles.getAccountRoles(userWallet.address)).to.equal(accountRoles);
            expect(await testRoles.getAccountRoles(ownerWallet.address)).to.equal(toAccountRoles([1, 3]));
        });
        ...
        it('should propose a new role and accept it', async () => {
            ...
            expect(await testRoles.getProposedRoles(ownerWallet.address, userWallet.address)).to.equal(accountRoles);
            ...
            expect(await testRoles.getProposedRoles(ownerWallet.address, userWallet.address)).to.equal(0); // cleared proposed roles
            expect(await testRoles.getAccountRoles(userWallet.address)).to.equal(accountRoles); // 12 is the binary representation of roles [2, 3]
            expect(await testRoles.getAccountRoles(ownerWallet.address)).to.equal(toAccountRoles([1, 3]));
        });
    });
});
