...
const { deployContractConstant, deployAndInitContractConstant, predictContractConstant } = require('../../index.js');
...
describe('ConstAddressDeployer', () => {
    ...
    before(async () => {
        ...
        deployerFactory = await ethers.getContractFactory('ConstAddressDeployer', deployerWallet);
    });
    ...
    describe('deploy', () => {
        it('should deploy to the predicted address', async () => {
            ...
            const address = await predictContractConstant(deployer, userWallet, BurnableMintableCappedERC20, key, [
                name,
                symbol,
                decimals,
            ]);
            const contract = await deployContractConstant(deployer, userWallet, BurnableMintableCappedERC20, key, [
                name,
                symbol,
                decimals,
            ]);
    ...
    describe('deployAndInit', () => {
        it('should deploy to the predicted address regardless of init data', async () => {
            const key = 'a test key';
            const address = await predictContractConstant(deployer, userWallet, BurnableMintableCappedERC20Init, key, [
                decimals,
            ]);
    ...
    describe('should preserve the bytecode [ @skip-on-coverage ]', () => {
        it('should preserve the deployer bytecode', async () => {
            ...
            const expected = {
                istanbul: '0x79d0b7d0f663b2c3f60a1d05538d99b3245c362123a6671132afae524f7b5d45',
                berlin: '0x44db6b0a90d8beb5c3c5c53685a4ef195dfec7ffa71d24e90963f19fbd00937e',
                london: '0x3ac0db9ec02bb762514fb942c0223dff7a1f2fd0f9bb4df3fba8dba6a20c616f',
            }[getEVMVersion()];
