...
describe('LibsTest', () => {
    ...
    before(async () => {
        ...
        noFallBackFactory = await ethers.getContractFactory('NoFallback', ownerWallet);
    });
    ...
    it('should convert address to lowercase string', async () => {
        ...
        expect(await libsTest.addressToString(address)).to.equal(address.toLowerCase());
    });

    it('should convert string of any format to address', async () => {
        const address = ownerWallet.address;
        expect(await libsTest.stringToAddress(address)).to.equal(address);
        expect(await libsTest.stringToAddress(address.toString())).to.equal(address);
        expect(await libsTest.stringToAddress(address.toString().toLowerCase())).to.equal(address);
    });

    it('should revert on invalid string to address conversion', async () => {
        let invalidAddressString = '0x123';
        await expect(libsTest.stringToAddress(invalidAddressString)).to.be.revertedWithCustomError(
            libsTest,
    ...
    it('should convert string to bytes and back', async () => {
        ...
        expect(bytes).to.equal('0x626967207465737420737472696e67000000000000000000000000000000000f');
        expect(await libsTest.bytes32ToString(bytes)).to.equal(string);
    });

    it('should revert on invalid string to bytes conversion', async () => {
        let invalidString = '';
        await expect(libsTest.stringToBytes32(invalidString)).to.be.revertedWithCustomError(
            libsTest,
            'InvalidStringLength',
        );
    ...
    it('should revert if safe native transfer fails', async () => {
        ...
        await expect(libsTest.nativeTransfer(noFallBack.address, value)).to.be.revertedWithCustomError(
            libsTest,
