...
const { create2DeployContract, create2DeployAndInitContract, getCreate2Address } = require('../../index.js');
...
describe('Create2Deployer', () => {
    ...
    before(async () => {
        ...
        deployerFactory = await ethers.getContractFactory('Create2Deployer', deployerWallet);
    });
    ...
    describe('deploy', () => {
        ...
        it('should deploy to the predicted address', async () => {
            const key = 'a test key';
            const address = await getCreate2Address(deployer, userWallet, BurnableMintableCappedERC20, key, [
                name,
                symbol,
                decimals,
            ]);

            const contract = await create2DeployContract(deployer, userWallet, BurnableMintableCappedERC20, key, [
                name,
                symbol,
                decimals,
            ]);
    ...
    describe('deployAndInit', () => {
        it('should deploy to the predicted address regardless of init data', async () => {
            const key = 'a test key';
            const address = await getCreate2Address(deployer, userWallet, BurnableMintableCappedERC20Init, key, [
                decimals,
            ]);
    ...
    describe('should preserve the bytecode [ @skip-on-coverage ]', () => {
        it('should preserve the deployer bytecode', async () => {
            ...
            const expected = {
                istanbul: '0x90d7fe2e549fbcb3bd68235f83166b5a749b6634ad174a84f71f201b2c2401b2',
                berlin: '0x5b6c1a919aab7bff15cc9d522898b9455e15c7047c2036acaf028de68f6612f4',
                london: '0xcb4f6fe9556b1697bdfcf1faada231f3a1e017ccf01374914927b7dbbb0daaa2',
            }[getEVMVersion()];
