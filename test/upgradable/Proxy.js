...
describe('Proxy', async () => {
    ...
    before(async () => {
        ...
        proxyImplementationFactory = await ethers.getContractFactory('ProxyImplementation', owner);
    ...
    describe('Proxy & BaseProxy', async () => {
        ...
        beforeEach(async () => {
            ...
            invalidSetupProxyImplementation = await invalidSetupProxyImplementationFactory
                .deploy()
                .then((d) => d.deployed());
        });
        ...
        it('reverts on invalid owner', async () => {
            await expect(
                proxyFactory.deploy(proxyImplementation.address, ethers.constants.AddressZero, '0x'),
        ...
        it('reverts on invalid implementation', async () => {
            ...
            await expect(
                proxyFactory.deploy(invalidProxyImplementation.address, owner.address, setupParams, getGasOptions()),
        ...
        it('should shadow a call to setup on the implementation', async () => {
            let setupParams = defaultAbiCoder.encode(['uint256', 'string'], [123, 'test']);
            ...
            setupParams = defaultAbiCoder.encode(['uint256', 'string'], [456, 'test']);
        ...
        it('should preserve the proxy bytecode [ @skip-on-coverage ]', async () => {
            ...
            const expected = {
                istanbul: '0x9708ed200e1c05090844fc16c500a593be639f493c8949ee2d773aa0fce7d051',
                berlin: '0xfc17f3b1332553428d12c6edcb24322e92f5ff4d455c0112845a249c936bf6fc',
                london: '0x04e1ba2679e69e7ce1a3934e9aecc4de17c178000c619658abe453ce74854105',
            }[getEVMVersion()];
    ...
    describe('FinalProxy', async () => {
        ...
        beforeEach(async () => {
            ...
            differentProxyImplementation = await differentProxyImplementationFactory.deploy().then((d) => d.deployed());
        });

        it('check internal constants', async () => {
            const setupParams = '0x';
            const testFinalProxyFactory = await ethers.getContractFactory('TestFinalProxy', owner);
        ...
        it('reverts when finalUpgrade is called by a non-owner', async () => {
            ...
            await expect(
                finalProxy.connect(user).finalUpgrade(differentProxyImplementation.address, setupParams),
        ...
        it('reverts if setup params are provided but implementation setup fails', async () => {
            ...
            setupParams = defaultAbiCoder.encode(['uint256', 'string'], [123, 'test']);

            const bytecode = await invalidSetupProxyImplementationFactory.getDeployTransaction().data;

            await expect(finalProxy.finalUpgrade(bytecode, setupParams)).to.be.revertedWithCustomError(
                finalProxy,
                'SetupFailed',
            );
        });
        ...
        it('should revert on final upgrade with invalid contract ID', async () => {
            ...
            const bytecode = await invalidProxyImplementationFactory.getDeployTransaction().data;
        ...
        it('after finalUpgrade, isFinal returns true', async () => {
            ...
            const bytecode = await differentProxyImplementationFactory.getDeployTransaction().data;
        ...
        it('should perform final upgrade with setup params', async () => {
            ...
            finalProxy = await finalProxyFactory
                .deploy(differentProxyImplementation.address, owner.address, setupParams)
                .then((d) => d.deployed());

            const bytecode = await proxyImplementationFactory.getDeployTransaction().data;
        ...
        it('reverts on second call to finalUpgrade', async () => {
            ...
            let bytecode = await differentProxyImplementationFactory.getDeployTransaction().data;
        ...
        it('should preserve the final proxy bytecode [ @skip-on-coverage ]', async () => {
            ...
            const expected = {
                istanbul: '0x570e4c4cc995c99cf9057ac71c4136d102f06889927bc1dd288a1d3457a82cc3',
                berlin: '0x3b58c148b79cf0e98e5217a227622a25a7aab91c752a2f2b3f53c89dcbca929d',
                london: '0xd2488fd23a1380e0836ef6b9f562fc80d2196d3f8059e33eb39b3af3b01da254',
            }[getEVMVersion()];
    ...
    describe('InitProxy', async () => {
        ...
        it('should revert if non-owner calls init', async () => {
            await expect(
                initProxy.connect(user).init(proxyImplementation.address, owner.address, '0x'),
            ).to.be.revertedWithCustomError(initProxy, 'NotOwner');
        });

        it('should initialize init proxy with the implementation', async () => {
            await initProxy.init(proxyImplementation.address, owner.address, '0x').then((tx) => tx.wait());
        ...
        it('should revert with invalid contract ID', async () => {
            initProxyFactory = await ethers.getContractFactory('TestInitProxy', owner);
            ...
            await expect(
                initProxy.init(invalidProxyImplementation.address, owner.address, setupParams),
        ...
        it('should initialize init proxy with setup params', async () => {
            ...
            const setupParams = defaultAbiCoder.encode(['uint256', 'string'], [initVal, initName]);

            // if init does not revert, setup was successful
            await initProxy.init(proxyImplementation.address, owner.address, setupParams).then((tx) => tx.wait());
        });
        ...
        it('should preserve the init proxy bytecode [ @skip-on-coverage ]', async () => {
            ...
            const expected = {
                istanbul: '0x75dc062062acecd55d475213b60dfd69753017317a5ce14078758d1ccb32f17f',
                berlin: '0x8d4322fbdf6c1488dabe92d991e3e74f3b1666184b22110e1f1f924bc1db2e8b',
                london: '0x107eefebd032510da85d8de3712d95bab49956c66f9bd79a280bb736e6cfd054',
            }[getEVMVersion()];
