...
describe('InterchainGovernance', () => {
  ...
  before(async () => {
    ...
    interchainGovernanceFactory = await ethers.getContractFactory(
      'InterchainGovernance',
      ownerWallet,
    );

    targetFactory = await ethers.getContractFactory('Target', ownerWallet);
    targetInterface = new ethers.utils.Interface(
      targetFactory.interface.fragments,
    );
    ...
    const mockGatewayFactory = await ethers.getContractFactory(
      'MockGatewayValidation',
      ownerWallet,
    );
    gatewayAddress = await mockGatewayFactory
      .deploy()
      .then((d) => d.deployed());
  });
  ...
  describe('contructor checks', () => {
    it('should revert on invalid constructor args', async () => {
      await expect(
        interchainGovernanceFactory.deploy(
          AddressZero,
          governanceChain,
  ...
  describe('negative tests', () => {
    ...
    it('should revert on calling withdraw directly', async () => {
      ...
      const nativeValue = 100;
      await expect(
        interchainGovernance.withdraw(recipient, nativeValue),
      ).to.be.revertedWithCustomError(interchainGovernance, 'NotSelf');
    });
    ...
    it('should revert on executing a proposal if call to target fails', async () => {
      ...
      await expect(
        interchainGovernance.executeProposal(
          target,
          invalidCalldata,
          nativeValue,
          { value: nativeValue },
    ...
    it('should revert on execute with token', async () => {
      await expect(
        interchainGovernance.executeWithToken(
          govCommandID,
          governanceChain,
          AddressZero,
          '0x',
  ...
  describe('positive tests', () => {
    ...
    it('should schedule a proposal and get the correct eta', async () => {
      ...
      const proposalEta = await interchainGovernance.getProposalEta(
        target,
        calldata,
        nativeValue,
      );
    ...
    it('should withdraw native ether from governance', async () => {
      ...
      const tx = await interchainGovernance.executeProposal(
        target,
        calldata,
        nativeValue,
      );
      ...
      expect(
        await ethers.provider.getBalance(interchainGovernance.address),
      ).to.equal(0);
    });
    ...
    it('should withdraw native ether from governance to recipient via withdraw method', async () => {
      ...
      const withdrawInterface = new ethers.utils.Interface(
        interchainGovernance.interface.fragments,
      );
      const withdrawCalldata = withdrawInterface.encodeFunctionData(
        'withdraw',
        [recipient, nativeValue],
      );
