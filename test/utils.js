...
const getPayloadAndProposalHash = async (commandID, target, nativeValue, calldata, timeDelay) => {
    ...
    const proposalHash = keccak256(
        defaultAbiCoder.encode(['address', 'bytes', 'uint256'], [target, calldata, nativeValue]),
    );
...
const expectRevert = async (txFunc, contract, error, args) => {
    if (network.config.skipRevertTests) {
        ...
        await expect(txFunc(null)).to.be.revertedWithCustomError(contract, error, args);
    }
};
...
module.exports = {
    ...
    getSignedExecuteInput: (data, wallet) =>
        wallet
            ...
            .then((signature) => defaultAbiCoder.encode(['bytes', 'bytes'], [data, signature])),
