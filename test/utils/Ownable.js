...
describe('Ownable', () => {
    ...
    before(async () => {
        ...
        ownableTestFactory = await ethers.getContractFactory('TestOwnable', ownerWallet);
    });

    describe('negative tests', () => {
        before(async () => {
            ownableTest = await ownableTestFactory.deploy(ownerWallet.address).then((d) => d.deployed());
        });

        it('should revert when non-owner calls only owner function', async () => {
            const num = 5;

            await expect(ownableTest.connect(userWallet).setNum(num)).to.be.revertedWithCustomError(
                ownableTest,
                'NotOwner',
            );
        });
        ...
        it('should not revert when owner calls only owner function', async () => {
            ...
            await expect(ownableTest.connect(ownerWallet).setNum(num)).to.emit(ownableTest, 'NumAdded').withArgs(num);
        });

        it('should revert on transfer owner if not called by the current owner', async () => {
            const newOwner = userWallet.address;

            await expect(ownableTest.connect(userWallet).transferOwnership(newOwner)).to.be.revertedWithCustomError(
                ownableTest,
                'NotOwner',
            );
        });
        ...
        it('should revert on transfer owner if new owner address is invalid', async () => {
            await expect(ownableTest.connect(ownerWallet).transferOwnership(AddressZero)).to.be.revertedWithCustomError(
                ownableTest,
                'InvalidOwnerAddress',
            );
        });
        ...
        it('should revert on propose owner if not called by the current owner', async () => {
            ...
            await expect(ownableTest.connect(userWallet).proposeOwnership(newOwner)).to.be.revertedWithCustomError(
                ownableTest,
                'NotOwner',
            );
        });

        it('should revert on propose ownership if new owner address is invalid', async () => {
            await expect(ownableTest.connect(ownerWallet).proposeOwnership(AddressZero)).to.be.revertedWithCustomError(
                ownableTest,
                'InvalidOwnerAddress',
            );
        });
        ...
        it('should revert on accept ownership if caller is not the pending owner', async () => {
            const ownableTest = await ownableTestFactory.deploy(ownerWallet.address).then((d) => d.deployed());
            ...
            await expect(ownableTest.connect(ownerWallet).acceptOwnership()).to.be.revertedWithCustomError(
                ownableTest,
                'InvalidOwner',
            );
        });
        ...
        it('should revert on accept ownership if transfer ownership is called first', async () => {
            ...
            await expect(ownableTest.connect(userWallet).acceptOwnership()).to.be.revertedWithCustomError(
                ownableTest,
                'InvalidOwner',
            );
        });
    ...
    describe('positive tests', () => {
        ...
        it('should propose new owner and accept ownership', async () => {
            const newOwner = userWallet.address;
