...
describe('Pausable', () => {
    ...
    it('Should be able to set paused to true or false', async () => {
        await expect(test.pause()).to.emit(test, 'Paused').withArgs(ownerWallet.address);
        expect(await test.paused()).to.equal(true);
        await expect(test.unpause()).to.emit(test, 'Unpaused').withArgs(ownerWallet.address);
        expect(await test.paused()).to.equal(false);
    });

    it('Should be able to execute notPaused functions only when not paused', async () => {
        await expect(test.pause()).to.emit(test, 'Paused').withArgs(ownerWallet.address);
        await expect(test.testPaused()).to.be.revertedWithCustomError(test, 'Pause');
    ...
    it('Should be able to execute paused functions only when paused', async () => {
        ...
        await expect(test.unpause()).to.emit(test, 'Unpaused').withArgs(ownerWallet.address);
        await expect(test.testNotPaused()).to.be.revertedWithCustomError(test, 'NotPaused');
    });
});
