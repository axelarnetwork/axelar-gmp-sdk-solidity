...
describe('BaseMultisig', () => {
    ...
    before(async () => {
        ...
        rotatedAccounts = [signer2, signer3, nonSigner].map((signer) => signer.address);

        multisigFactory = await ethers.getContractFactory('TestBaseMultisig', signer1);
    });

    describe('queries', () => {
        before(async () => {
            multisig = await multisigFactory.deploy(initAccounts, threshold).then((d) => d.deployed());
        });
    ...
    describe('negative tests', () => {
        ...
        it('should revert on rotate signers with any duplicate signers', async () => {
            ...
            const rotatedAccountsWithDuplicate = rotatedAccounts.concat(rotatedAccounts[0]);
            ...
            await expect(
                multisig.connect(signer2).rotateSigners(rotatedAccountsWithDuplicate, newThreshold),
        ...
        it('should revert on rotate signers with any invalid signer addresses', async () => {
            ...
            await expect(
                multisig.connect(signer2).rotateSigners(rotatedAccountsInvalid, newThreshold),
            ).to.be.revertedWithCustomError(multisig, 'InvalidSigners');
        });
    });
    ...
    describe('positive tests', () => {
        ...
        it('should proceed with operation execution with sufficient votes', async () => {
            ...
            const msgData = multisig.interface.encodeFunctionData('rotateSigners', [rotatedAccounts, newThreshold]);
            const msgDataHash = keccak256(msgData);

            await expect(await multisig.connect(signer1).rotateSigners(rotatedAccounts, newThreshold))
        ...
        it('should reset the votes internally', async () => {
            ...
            expect(await multisig.hasSignerVoted(signer1.address, msgDataHash)).to.equal(false);
            ...
            expect(await multisig.hasSignerVoted(signer1.address, msgDataHash)).to.equal(true);
            ...
            await expect(await multisig.connect(signer2).rotateSigners(rotatedAccounts, newThreshold))
                .to.emit(multisig, 'MultisigOperationExecuted')
                .withArgs(msgDataHash, 1, signer2.address, 2);
        });
