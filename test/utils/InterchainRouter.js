...
describe.only('InterchainRouter', () => {
    ...
    before(async () => {
        ...
        const implementation = await deployContract(ownerWallet, 'InterchainRouter', [chainName]);
        const params = defaultAbiCoder.encode(['string[]', 'string[]'], [defaultChains, defaultAddresses])
        interchainRouter = await deployContract(ownerWallet, 'TestInterchainRouterProxy', [implementation.address, ownerWallet.address, params]);
        interchainRouter = new Contract(interchainRouter.address, InterchainRouter.abi, ownerWallet);
    });

    it('Should revert on interchainRouter deployment with invalid chain name', async () => {
        const interchainRouterFactory = await ethers.getContractFactory('InterchainRouter');
        await expect(
    ...
    it('Should revert on interchainRouter deployment with length mismatch between chains and trusted addresses arrays', async () => {
        const interchainRouterImpl = await deployContract(ownerWallet, 'InterchainRouter', [chainName]);
        const interchainRouterProxyFactory = await ethers.getContractFactory('TestInterchainRouterProxy');
        const params = defaultAbiCoder.encode(['string[]', 'string[]'], [['Chain A'], []]);
        await expect(
            interchainRouterProxyFactory.deploy(interchainRouterImpl.address, ownerWallet.address, params)
        ).to.be.revertedWithCustomError(
            interchainRouter,
            'SetupFailed',
        );
    ...
    it('Should revert when querrying the remote address for unregistered chains', async () => {
        await expect(interchainRouter.getRemoteAddress(otherChain)).to.be.revertedWithCustomError(
            interchainRouter,
            'UntrustedChain',
        );
    });
    ...
    it('Should be able to validate remote addresses properly', async () => {
        expect(await interchainRouter.validateSender(otherChain, otherRemoteAddress)).to.equal(false);
    });

    it('Should not be able to add a custom remote address as not the owner', async () => {
        await expect(
            interchainRouter.connect(otherWallet).addTrustedAddress(otherChain, otherRemoteAddress)
        ).to.be.revertedWithCustomError(
            interchainRouter,
            'NotOwner',
        );
    ...
    it('Should be able to add a custom remote address as the owner', async () => {
        await expect(interchainRouter.addTrustedAddress(otherChain, otherRemoteAddress))
            .to.emit(interchainRouter, 'TrustedAddressAdded')
            .withArgs(otherChain, otherRemoteAddress);
        expect(await interchainRouter.getRemoteAddress(otherChain)).to.equal(otherRemoteAddress);
    });
    ...
    it('Should be able to validate remote addresses properly.', async () => {
        expect(await interchainRouter.validateSender(otherChain, otherRemoteAddress)).to.equal(true);
    });
    ...
    it('Should revert on removing a custom remote address with an empty chain name', async () => {
        await expect(interchainRouter.removeTrustedAddress(''))
            .to.be.revertedWithCustomError(
            interchainRouter,
            'ZeroStringLength',
        );
