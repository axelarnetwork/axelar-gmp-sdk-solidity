...
describe('Distributable', () => {
  let test;
  before(async () => {
    test = await deployContract(ownerWallet, 'DistributableTest', [
      ownerWallet.address,
    ]);
  });
  ...
  it('Should not be able to run the onlyDistributor function as not the distributor', async () => {
    await expect(
      test.connect(otherWallet).testDistributable(),
    ).to.be.revertedWithCustomError(test, 'NotDistributor');
  });
  ...
  it('Should be able to change the distributor only as the distributor', async () => {
    ...
    await expect(
      test.transferDistributorship(otherWallet.address),
    ).to.be.revertedWithCustomError(test, 'NotDistributor');
  });
});
